...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim){
	if (htim == &htim4){
		sensorCounter++; // if sensorCounter is 1, takes 7 seconds to send data of all sensor
			if (sensorCounter >= 1) {
				sensorCounter= 0;
				sensorFlag=1;
				}
...
void I2C_Scanner(I2C_HandleTypeDef *hi2c) {
    ...
    for (i = 1; i < 128; i++) { // 7-bit I2C adresleri
        result = HAL_I2C_IsDeviceReady(&hi2c1, (uint16_t)(i << 1), 1, 10);
        if (result == HAL_OK) { // Eğer cihaz algılandıysa
            sprintf(msg, "0x%X cihaz algılandı!\r\n", i << 1);
            HAL_UART_Transmit(&huart2, (uint8_t *)msg, strlen(msg), HAL_MAX_DELAY);
        } else { // Cihaz algılanamadı
            sprintf(msg, "0x%X cihaz algılanamadı.\r\n", i << 1);
            HAL_UART_Transmit(&huart2, (uint8_t *)msg, strlen(msg), HAL_MAX_DELAY);
        }
    }
}
...
/***** SYSTEMS*****/
void irrigationSystem(uint8_t minSF){

	//working with rtc and configure the time of system, main funciton of sf
	if(7<= minSF && minSF<16){
		HAL_GPIO_WritePin(GPIOE, GPIO_PIN_13, GPIO_PIN_SET);
		flagIR = 1;
	}
	else if(17<=minSF && minSF<=24){
		HAL_GPIO_WritePin(GPIOE, GPIO_PIN_14, GPIO_PIN_SET);
		HAL_GPIO_WritePin(GPIOE, GPIO_PIN_13, GPIO_PIN_RESET);
		flagIR = 2;
	}
...
int main(void)
{
  ...
  HAL_UART_Transmit(&huart2, (uint8_t *)"I2C tarama başlıyor...\r\n", 24, HAL_MAX_DELAY);
