...
void irrigationSystem(uint16_t minSF){
	...
	if(102<= minSF && minSF<=109){ // Water to outside
		HAL_GPIO_WritePin(GPIOE, GPIO_PIN_14, GPIO_PIN_RESET);
	    HAL_GPIO_WritePin(GPIOE, GPIO_PIN_13, GPIO_PIN_SET);
		flagIR = 1;
	}
	else if(112<=minSF && minSF<=119){ // Water to inside
...
{
	timerVariable2++;
	if(timerVariable2 >= 1441){

		timerVariable2 = 0;
	}
	lightingSystem(timerVariable2);
}
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
  if(huart == &huart2){
	  ...
	  switch(command){
	  	  case 11: // water pump inside start
	  		  rxFlag = 1;
	  		  HAL_GPIO_WritePin(GPIOE, GPIO_PIN_13, GPIO_PIN_RESET);
	  		  HAL_GPIO_WritePin(GPIOE, GPIO_PIN_14, GPIO_PIN_SET);
	  		  break;
	  	  case 10: // water pump all start
	  		  rxFlag = 0;
	  		  HAL_GPIO_WritePin(GPIOE, GPIO_PIN_13, GPIO_PIN_SET);
	  		  HAL_GPIO_WritePin(GPIOE, GPIO_PIN_14, GPIO_PIN_SET);
	  		  break;
	  	  case 21: // water pump outside start
	  		  rxFlag = 1;
	  		  HAL_GPIO_WritePin(GPIOE, GPIO_PIN_13, GPIO_PIN_SET);
	  		  HAL_GPIO_WritePin(GPIOE, GPIO_PIN_14, GPIO_PIN_RESET);
	  		  break;
...
void sensorData(){
		...
		getDHT11(&temp2, &hum2);
		sprintf((char*)loraBuffer,"%.1f,%.1f,%.1f,%.1f,%.1f,%d,%d,%d,%d,%d,%d,%d\n",temp2,hum2,airQ,waterTemp,wL,hour,min,sec,flagIR,flagVENT,flagLIGH,flagHEAT);
...
void ventilationSystem (uint8_t nem){
...
/* USER CODE END MX_GPIO_Init_1 */
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOB, LED_Pin|Besleme_Pin, GPIO_PIN_RESET);
  ...
  /*Configure GPIO pins : LED_Pin Besleme_Pin */
  GPIO_InitStruct.Pin = LED_Pin|Besleme_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
